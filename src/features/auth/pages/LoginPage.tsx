...
export function LoginPage() {
  ...
  const submit = async (values: LoginFormValues) => {
    try {
      ...
      setError('root', { message: appError.message || 'Identifiants incorrects.' })
    }
  }
  ...
  return (
    <section className="flex min-h-screen items-center justify-center bg-arena-950 px-4">
      <div className="w-full max-w-md rounded-xl border border-slate-800 bg-arena-900 p-6 shadow-xl shadow-black/20">
        ...
        <p className="mt-1 text-sm text-slate-400">Accède à ton espace tournoi sécurisé.</p>
        ...
        <p className="mt-4 text-sm text-slate-400">
          ...
          <Link to="/register" className="font-medium text-neon-400 hover:text-sky-300">
            Créer un compte
          </Link>
        </p>
      </div>
